...
//device tree structure taew00k.kang [start]
...
#include <mach/board_lge.h> //                                                            
//device tree structure taew00k.kang [end]
...
static struct wake_lock		oneseg_wake_lock;	/* wake_lock,wake_unlock */
...
static struct of_device_id mb86a36k_i2c_table[] = {
	{ .compatible = "fujitsu,mb86a36k-i2c" ,}, // Compatible node must match dts
	{ },
};
#else
...
#define	PLLF_VHF		0x064924
	...
	u8 VCORG,REG06=0,REG86; //WBT #387118
	u8 ui8REVID;
...
/************************************************************************/
/**
          ioctl System Call.  IOCTL_SET_SOFT_PDOWN command control. \n
          Device Driver for Multi mode tuner module. (MB86A35)
		      ...
		      unsigned long arg)
{
  ...
  int rtncode = 0;
   ...
   if (chipid0_rf == 0x03) {    //ES2.0 start
	...
	// removed for A36S because it is the initial value
	...
	case PARAM_MODE_ISDBTSB_1VHF:	/* ISDB-Tsb  1seg / VHF */
		DTSL = 0x0A;
		AGAIN = 0x5D;  //changed from 0x48 for v0.7
	...
	case PARAM_MODE_ISDBTMM_13VHF:	/* ISDB-Tmm 13seg / VHF */
		...
		VIFREFL = 0x08;  //changed from 0x40 for v0.5
		IFSAMPLE = 0x01;
		OUTSAMPLE = 0x01;
		break;
	default:
		       ...
		       unsigned long arg)
{
  ...
  u8 chipid0_rf = 0;
  int rtncode = 0;
//  u8 data[4];	// build error fix used
   rtncode_rf = mb86a35_i2c_rf_recv(reg_rf, &chipid0_rf, 1);
   if (rtncode_rf != 0) {
    		rtncode = -EFAULT;
    		goto sync_return;
   ...
   if (chipid0_rf == 0x03) {    //ES2.0 start
	...
	int indx;
		...
		// setting for 1seg (UHF)
		MB86A35_setting_data_t syncdata[] = {
		////////////////// Below settings are removed for A36S
		//{0x28, 0xEC}, {0x29, 0x41}, {0x2A, 0x40}, {0x2B, 0x10},
		//{0x28, 0xEE}, {0x29, 0xC0}, {0x2A, 0x04}, {0x2B, 0x00},
		//{0x28, 0x64}, {0x29, 0x00}, {0x2A, 0x00}, {0x2B, 0x80},
		//{0x3B, 0xD1}, {0x3C, 0xA1},
		//{0x3B, 0xD7}, {0x3C, 0x00},
		//{0x28, 0x20}, {0x29, 0x56}, {0x2A, 0x24}, {0x2B, 0x4A}, //ES3.0 1seg(32.571MHz) only, moved to RF_channel
		...
		////////////////// Above settings are removed for A36S

		{0x04, 0x0A}, {0x05, 0xFF},  // 1seg only (modified for v0.7)

		{0xD0, 0x02},  // 1seg only
		{0xD9, 0xDA},  // 1seg only

		//{0x28, 0xD0}, {0x29, 0x04}, {0x2A, 0x00}, {0x2B, 0x60}, // A36S 1seg UHF only (removed for v0.5)
		//{0x28, 0xCF}, {0x29, 0x00}, {0x2A, 0x03}, {0x2B, 0x80}, // A36S 1seg UHF only (removed for v0.5)
		
		{0xff, 0xff}
		};
	
		// setting for 13seg (UHF/VHF)
		...
		{0x04, 0x0A}, {0x05, 0xC9},  // 13seg only (modified for v0.5)

		{0xD0, 0x00},  //13seg only
		{0xD9, 0xD8},  //13seg only

		//{0x28, 0xD0}, {0x29, 0x04}, {0x2A, 0x00}, {0x2B, 0x68}, // VHF only (removed tofor v0.5)
		{0x28, 0xCF}, {0x29, 0x00}, {0x2A, 0x01}, {0x2B, 0x80}, // 13seg only
									// VHF seg#7 or #20 setting is in RF_channel
		
		{0xff, 0xff}
		};

		// setting for 1seg (VHF)
		...
		//{0x28, 0xD0}, {0x29, 0x04}, {0x2A, 0x00}, {0x2B, 0x68}, // VHF only (removed for v0.5)
		//{0x28, 0xCF}, {0x29, 0x00}, {0x2A, 0x02}, {0x2B, 0x80}, // VHF 1seg only (removed for v0.5)
	...
	// Common settings for optimization of multipath
	...
	//mb86a35_i2c_slave_send(0x6d, 0xf9, 0x6e, 0xbf);
	//mb86a35_i2c_slave_send(0x6d, 0xfb, 0x6e, 0xbf);
...
//		{0x50, 0x38}, {0x51, 0xA0}, // removed for A36S 
		{0xff, 0xff}
	};
	int indx;
			     ...
			     unsigned long arg)
{
	...
	size_t tmpsize = sizeof(ioctl_rf_t);
	int rtncode = 0;
//  	u8 tmpdata = 0; // unused variable build error fix - taew00k.kang
	unsigned char reg;
//	unsigned int value; // unused variable build error fix - taew00k.kang
	...
	case PARAM_MODE_ISDBT_1UHF:
		...
		// FREQSEL=0, QDEMOD_OFF = 0x4BB442, 2nd_MIX_DIV = 0x30
		if ((RF->ch_no==15)
		 || (RF->ch_no==19)
		 || (RF->ch_no==27)
		 || (RF->ch_no==31)
		...
		// FREQSEL=1, QDEMOD_OFF = 0x49D5F6, 2nd_MIX_DIV = 0x20
		else if ((RF->ch_no==13)
		 || (RF->ch_no==21)
		 || (RF->ch_no==38)
		 || (RF->ch_no==40)
		...
		// FREQSEL=1, QDEMOD_OFF = 0x40E3A6, 2nd_MIX_DIV = 0x30
		else if ((RF->ch_no==23)
		 || (RF->ch_no==39)
		)
		{
		...
		// FREQSEL=2, QDEMOD_OFF = 0x43A3CC, 2nd_MIX_DIV = 0x20
		else if ((RF->ch_no==20)
		 || (RF->ch_no==22)
		 || (RF->ch_no==41)
		)
		...
		// FREQSEL=0, QDEMOD_OFF = 0x56244A,2nd_MIX_DIV = 0x20 
		else
		{
			mb86a35_i2c_master_send(0x71, 0x02 ) ;
	...
	case PARAM_MODE_ISDBTMM_1VHF:
		...
		// FREQSEL=1, QDEMOD_OFF = 0x43AECC, 2nd_MIX_DIV = 0x20
		else if (RF->ch_no==29) 
		{
			mb86a35_i2c_master_send(0x71, 0x12) ;
...
/*
	...
	case 0:	// inner
		gpio_request(GPIO_ISDBT_ANT_SELECT_1, "antenna_1");
		gpio_direction_output(GPIO_ISDBT_ANT_SELECT_1, 0);
		gpio_request(GPIO_ISDBT_ANT_SELECT_2, "antenna_2");
		gpio_direction_output(GPIO_ISDBT_ANT_SELECT_2, 1);
		break;
	case 1:	// ear
		gpio_request(GPIO_ISDBT_ANT_SELECT_1, "antenna_1");
		gpio_direction_output(GPIO_ISDBT_ANT_SELECT_1, 1);
		gpio_request(GPIO_ISDBT_ANT_SELECT_2, "antenna_2");
		gpio_direction_output(GPIO_ISDBT_ANT_SELECT_2, 0);
		break;
...
#endif
{
	...
	case IOCTL_SET_ALOG_PDOWN:	/* set Analog Power-down (DAC/ADC Power-down) */
		rtn = mb86a35_IOCTL_SET_ALOG_PDOWN(cmdctrl, cmd, arg);
		break;

	case IOCTL_SET_SOFT_PDOWN:	/* set Soft Power-down */
...
{
	int ret;
	int addr=-1;// Parsed	//taew00k.kang
	
	DBGPRINT(PRINT_LHEADERFMT "Called. \n", PRINT_LHEADER);

//	memset(&mhddCtrlInfo, 0x00, sizeof(struct MHDD_LG2102_CTRL));
	...
	// You can initiate a I2C transfer anytime using i2c_client * client structure

//	i2c_set_clientdata(client, (void*)&mb86a35_i2c_client);
